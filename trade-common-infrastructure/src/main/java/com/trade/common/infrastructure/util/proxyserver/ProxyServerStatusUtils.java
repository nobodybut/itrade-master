...
import com.trade.common.infrastructure.util.httpclient.OkHttpClientUtils;
	...
	public static void recordProxyStatus(String postUrl, String proxyServer, boolean isRemoveProxyServer, LocalTime startTime, Logger s_logger) {
		...
		if (!Strings.isNullOrEmpty(proxyServerKV.getValue())) {
			...
			if (isRemoveProxyServer) {
				// 记录代理服务器错误数量
				OkHttpClientUtils.getHTML(String.format("http://www.itrade.work/proxyServerService/delProxyServer?proxyServerSupplier=%s&domain=%s&proxyServer=%s", proxyServerKV.getKey().ordinal(), byDomain, proxyServer));
				...
				// 记录代理服务器正确数量
				OkHttpClientUtils.getHTML(String.format("http://www.itrade.work/proxyServerService/incrSuccessProxyServer?proxyServerSupplier=%s&domain=%s", proxyServerKV.getKey().ordinal(), byDomain));
			}
		}
	}
