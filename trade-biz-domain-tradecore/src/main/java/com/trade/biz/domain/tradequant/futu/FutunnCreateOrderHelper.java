...
@Component
@Slf4j
public class FutunnCreateOrderHelper {
	...
	@Resource
	private FutunnCancelOrderHelper futunnCancelOrderHelper;
	 ...
	 * @return
	 */
	public boolean createQuantOrder(long stockID, String stockCode, TradeSideEnum tradeSide, float price, int volume) {
		...
		try {
			...
			// 循环检查交易是否真实成功
			boolean tradingIsSuccess = checkTradingIsSuccess(webDriver, stockID, stockCode, tradeSide, price, volume);
	 ...
	 * @return
	 */
	private boolean checkTradingIsSuccess(WebDriver webDriver, long stockID, String stockCode, TradeSideEnum tradeSide, float price, int volume) {
		try {
		...
		// 处理取消订单
		boolean cancelSuccess = futunnCancelOrderHelper.cancelQuantOrder(stockID, stockCode, "");
		if (!cancelSuccess) {
			log.error("cancelQuantOrder FAIL!");
		}
